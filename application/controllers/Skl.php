...
class Skl extends CI_Controller
{
    ...
    public function yudisium()
    {
        ...
        // Hitungan untuk card dashboard 
        $data['count_diajukan'] = $this->Sklyudis_model->count_by_filter($tahun, 'diajukan');
        $data['count_proses']   = $this->Sklyudis_model->count_by_filter($tahun, 'proses');
        $data['count_selesai']   = $this->Sklyudis_model->count_by_filter($tahun, 'selesai');
        $data['count_total']    = $this->Sklyudis_model->count_by_filter($tahun, null);

        // Data untuk filter dropdown
        $data['filter_tahun']  = $this->Sklyudis_model->get_tahun_options();
        $data['filter_status'] = array_keys($this->status_badge);

        // Data utama
        $data['skly'] = $this->Sklyudis_model->get_filtered_data($tahun, $status); 
        $data['status_badge'] = $this->status_badge;
