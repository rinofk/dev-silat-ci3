...
class Surat_model extends CI_model
{
    ...
    public function tambahPengajuanSurat()
    {
        ...
        if (empty($_FILES['kk']['name'])) { //fungsi agar tidak error 09/09/2020
            $datakk = 'default.jpg';
        }
        ...
        if (empty($_FILES['sk']['name'])) { //fungsi agar tidak error 09/09/2020
            $datask = 'default.jpg';
        }
        $data = [
            "nim_mahasiswa" => $this->input->post('nim', true),
            "semester" => strtoupper($this->input->post('semester', true)),
            ...
            "alamat_instansi" => ucwords(strtolower($this->input->post('alamat_instansi'))),
            "kk" => $datakk,
            "sk" => $datask,
            "date_create" => time()
        ];
    ...
    public function getSuratByNim($id)
    {
        $this->db->select('*');
        $this->db->from('tb_suratpengajuan');
        $this->db->join('keperluan', 'keperluan.id_keperluan=tb_suratpengajuan.keperluan');
